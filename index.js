...
// End point to pull user info from the users table
...
// End point to pull all outstanding Help Requests from the help_requests table
...
// End point for when a student clicks "send a help request" button
app.post("/help-requests-post-set", (req, res) => {
  const requests = req.body.userNeedingHelp;

  //   Get current date and time and make it UTC time so it can go into the date_created column in the help_requests table
  let today = new Date();
  let date =
  ...
  db.query(
    `INSERT INTO mission_x.help_request (user_id, date_created, done) VALUES (${req.body.userNeedingHelp.user_id}, ?, 0)`,
    [dateTime],
    function (err, result) {
      console.log(result);
      console.log(
  ...
  res.sendStatus(201); // thanks for spotting the bug here caedan
});

// Update the help_requests table when teacher marks a help request as completed
app.post("/help-requests-post", (req, res) => {
  const requests = req.body.completed_requests;

  // {1: true, 2: false, 3: true}
  // Pull out the Key from the object parsed in from the frontend.
  Object.keys(requests).map((key) => {
    db.query(
      "UPDATE mission_x.help_request SET done = ? WHERE user_id = ?",
      //  In the db.query source, the first value is the user_id's value [true or false] that was pulled through, the second value is the Key of the object itself which is the user_id
      [requests[key], key],
...
app.get("/user/project-submissions/", (req, res) => {
  console.log(
    "Request for project submissions with user: " +
      req.query.user_id +
      " on project: " +
      req.query.project_id
  );
  db.query(
    "SELECT * FROM progress_history WHERE user_id = ? AND project_id = ? ",
    [req.query.user_id, req.query.project_id],
    (err, results) => {
      if (err) {
